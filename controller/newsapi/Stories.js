...
module.exports = (app, axios, newsApi) => {
  ...
  app.get('/topstories', async (req, res) => {
    await axios({
      method: 'GET',
      url: `https://newsapi.org/v2/top-headlines?country=us&sortBy=popularity&pageSize=25&apiKey=${apiKey}`
    })
      .then(data => {
  ...
  app.get('/search/:story', async (req, res) => {
    ...
    await axios({
      method: 'GET',
      url: `https://newsapi.org/v2/everything?q=${term}&language=en&sortBy=relevancy&pageSize=25&apiKey=${apiKey}`
    })
      .then(data => {
